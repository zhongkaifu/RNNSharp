...
namespace RNNSharp
{
    public class SimpleRNN : RNN
    {
        ...
        Matrix<double> mat_bptt_syn0_w;
        Matrix<double> mat_bptt_syn0_ph;
        Matrix<double> mat_bptt_synf;
        ...
        public void resetBpttMem()
        {
            ...
            bptt_hidden = CreateLayer((bptt + bptt_block + 1) * L1);
        ...
        public override void LearnBackTime(State state, int numStates, int curState)
        {
            ...
            for (int a = maxBptt; a > 0; a--)
            {
                ...
                for (int i = 0; i < L1; i++)
                {
                    bptt_hidden[a * L1 + i].cellOutput = bptt_hidden[(a - 1) * L1 + i].cellOutput;
                    bptt_hidden[a * L1 + i].er = bptt_hidden[(a - 1) * L1 + i].er;
                }
